...
import { Trash2, Grid, Plus, MousePointer2, Upload, ZoomIn, ZoomOut, Move, Ruler, X, Check, Hand, MousePointer, RotateCw, Layers, Palette, Download, Printer, Eye, SquareDashedBottom } from 'lucide-react';
...
const App = () => {
  ...
  const [projectName, setProjectName] = useState('Квартира №42');
  ...
  useEffect(() => {
    ...
    itemsWithRooms.forEach(item => {
        addToBom(item.name, item.price, item.sku, item.roomName);
    });
    ...
    groups.forEach(group => {
        ...
        if (frameData) {
            addToBom(frameData.name, frameData.price, '-', roomName, true);
        }
    });
  ...
  const handleDelete = () => {
    if (selectedType === 'item') {
        setItems(items.filter(i => i.uniqueId !== selectedId));
  ...
  const handleMouseMove = (e) => {
      ...
      if (isDrawingRoom && roomStartPos) {
          ...
          const width = Math.abs(pos.x - roomStartPos.x);
          const height = Math.abs(pos.y - roomStartPos.y);
          setCurrentDrawingRoom({ x, y, width, height });
          return;
      }
  ...
  const handleMouseUp = (e) => {
      ...
      if (isDrawingRoom && currentDrawingRoom) {
          if (currentDrawingRoom.width > 50 && currentDrawingRoom.height > 50) {
              const name = prompt("Введіть назву кімнати (наприклад: Кухня):", "Нова кімната");
              if (name) {
                  setRooms([...rooms, { ...currentDrawingRoom, id: Date.now().toString(), name }]);
              }
          }
  ...
  const startItemDrag = (e, item) => {
      ...
      setInternalDragItem({
          ...
          originalW: item.width, originalH: item.height
      });
  ...
  const handleExportCSV = () => {
      ...
      csvContent += "Назва,Артикул,Кількість,Ціна,Сума\n";
      
      const bomItems = bom.items || {};
      Object.entries(bomItems).forEach(([name, data]) => {
          const row = [
              `"${name}"`,
              `"${data.sku}"`,
              data.count,
      ...
      csvContent += `,,,ЗАГАЛОМ:,${bom.total}\n`;

      const encodedUri = encodeURI(csvContent);
      const link = document.createElement("a");
      link.setAttribute("href", encodedUri);
      link.setAttribute("download", `${projectName || 'project'}_spec.csv`);
    ...
    <div className={`flex h-screen flex-col bg-gray-100 font-sans text-gray-800 overflow-hidden ${isPreviewMode ? 'print-preview-mode' : ''} print:block print:h-auto print:overflow-visible`} onMouseUp={handleMouseUp} onMouseMove={handleMouseMove}>
      ...
      <header className="bg-white border-b h-14 flex items-center justify-between px-6 shadow-sm z-10 select-none screen-only">
        <div className="flex items-center gap-2">
          ...
          <span className="font-bold text-lg">ElectroPlan <span className="text-blue-600">Pro</span></span>
          <span className="ml-4 text-xs bg-green-100 text-green-800 px-2 py-0.5 rounded-full border border-green-200">v2.0 Zoning</span>
        </div>
        
        {/* TOOLBAR */}
        <div className="flex bg-gray-100 rounded-lg p-1 gap-1">
            <button onClick={() => setActiveTool('select')} className={`p-2 rounded ${activeTool === 'select' ? 'bg-white shadow text-blue-600' : 'text-gray-500'}`} title="Курсор"><MousePointer size={18}/></button>
            <button onClick={() => setActiveTool('hand')} className={`p-2 rounded ${activeTool === 'hand' ? 'bg-white shadow text-blue-600' : 'text-gray-500'}`} title="Рука"><Hand size={18}/></button>
            <div className="w-px bg-gray-300 mx-1"></div>
            <button onClick={() => setActiveTool('room')} className={`flex items-center gap-2 px-3 py-1.5 rounded text-sm transition-colors ${activeTool === 'room' ? 'bg-white text-purple-600 shadow-sm font-bold' : 'text-gray-600 hover:bg-gray-200'}`} title="Створити Кімнату"> <SquareDashedBottom size={18} /> Кімната </button>
        </div>
        ...
        <div className="flex gap-3 items-center">
             ...
             <input type="file" ref={fileInputRef} onChange={(e) => {
                 ...
                 if(file) { const r = new FileReader(); r.onload = (ev) => { setPlanImage(ev.target.result); setPan({x:0,y:0}); setScale(0.2); }; r.readAsDataURL(file); }
             }} className="hidden" />
            <button onClick={() => fileInputRef.current.click()} className="p-2 hover:bg-gray-100 rounded"> <Upload size={18} /> </button>
      ...
      <div className={`flex flex-1 overflow-hidden relative ${isPreviewMode ? 'print-content-wrapper overflow-visible h-auto block' : ''} print:block print:overflow-visible print:h-auto`}>
        ...
        <div className={`w-80 bg-white border-r flex flex-col shadow-lg z-10 select-none screen-only ${isPreviewMode ? 'hidden' : ''}`}>
           ...
           <div className="flex-1 overflow-y-auto p-4 space-y-3">
             {PRODUCTS.map(p => (
               <div key={p.id} draggable onDragStart={(e) => { if(activeTool!=='select') setActiveTool('select'); setDraggedProductFromMenu(p); }} className="flex items-center gap-3 p-2 border rounded hover:shadow cursor-grab bg-white">
                  <div className="w-8 h-8 rounded bg-gray-100 flex items-center justify-center text-xs text-gray-500 font-bold">{p.series}</div>
             ...
             <div className="mt-4 p-3 bg-purple-50 text-xs text-purple-800 rounded border border-purple-200">
                 Tip: Виберіть інструмент <b>"Кімната"</b> зверху і обведіть зону на плані. Товари автоматично згрупуються!
             </div>
           </div>
           {/* MINI BOM */}
           <div className="h-1/3 border-t bg-gray-50 flex flex-col">
               ...
               <div className="flex-1 overflow-y-auto p-3 text-xs">
                   {Object.keys(bom.byRoom).map(room => (
                       <div key={room} className="mb-3">
                           <div className="font-bold text-gray-700 border-b mb-1 pb-1">{room}</div>
                           {Object.entries(bom.byRoom[room]).map(([name, d]) => (
                               <div key={name} className="flex justify-between py-0.5"><span>{name}</span><span className="font-mono">{d.count}</span></div>
                           ))}
                       </div>
                   ))}
               </div>
        ...
        <div className={`flex-1 bg-gray-200 relative overflow-hidden flex items-center justify-center ${isPreviewMode ? 'bg-white block h-auto p-0 m-0' : ''} print:block print:bg-white print:w-full print:h-auto print:overflow-visible print:static`}>
            ...
            {/* PRINT TITLE */}
            <div className={`hidden mb-6 border-b pb-4 ${isPreviewMode ? 'block' : ''} print:block`}>
                <h1 className="text-3xl font-bold">{projectName}</h1>
                <p className="text-gray-500">План розташування електрофурнітури</p>
            </div>

            {/* CANVAS */}
            <div className={`w-full h-full relative overflow-hidden ${isPreviewMode ? 'h-[500px] border border-gray-300 mb-8' : ''} print:h-[500px] print:w-full print:border print:border-gray-300 print:mb-8`} 
                 style={{ cursor: activeTool==='hand' || isPanning ? 'grabbing' : activeTool==='room' ? 'crosshair' : 'default' }}
                 onMouseDown={handleMouseDown} onContextMenu={e => e.preventDefault()} onWheel={(e) => { if(e.ctrlKey){e.preventDefault(); setScale(Math.max(0.01, scale - e.deltaY*0.001))} }}
                 onDrop={handleDropOnCanvas} onDragOver={e => e.preventDefault()}
            >
                <svg ref={svgRef} width="100%" height="100%" className="block">
                    <g transform={`translate(${pan.x}, ${pan.y}) scale(${scale})`}>
                        ...
                        {/* ROOMS LAYER (Under items) */}
                        {rooms.map(room => (
                            <g key={room.id} onClick={(e) => { if(activeTool==='select'){ e.stopPropagation(); setSelectedId(room.id); setSelectedType('room'); } }}>
                                <rect 
                                    x={room.x} y={room.y} width={room.width} height={room.height}
                                    fill={selectedId === room.id ? "rgba(37, 99, 235, 0.2)" : "rgba(37, 99, 235, 0.05)"}
                                    stroke="#2563eb" strokeWidth={4/scale} strokeDasharray={`${20/scale} ${10/scale}`}
                                />
                                <text x={room.x + 10} y={room.y + 40/scale} fontSize={40/scale} fill="#2563eb" fontWeight="bold" pointerEvents="none">{room.name}</text>
                            </g>
                        ))}
                        {currentDrawingRoom && (
                            <rect x={currentDrawingRoom.x} y={currentDrawingRoom.y} width={currentDrawingRoom.width} height={currentDrawingRoom.height} fill="rgba(37, 99, 235, 0.1)" stroke="#2563eb" strokeWidth={2/scale} />
                        )}
                        ...
                        {items.map(item => (
                            <g key={item.uniqueId} transform={`translate(${item.x}, ${item.y}) rotate(${item.rotation || 0}, ${item.width/2}, ${item.height/2})`} onMouseDown={e => startItemDrag(e, item)}>
                               <ProductShape item={item} isSelected={selectedId === item.uniqueId} />
                            </g>
                        ))}

                        {/* FRAMES */}
                        {detectedFrames.map((group, i) => {
                            let minX = Infinity, minY = Infinity, maxX = -Infinity, maxY = -Infinity;
                            const series = group[0].series || 'E2';
                            const color = group[0].color || '#1a1a1a';
                            group.forEach(item => { const b = getItemBounds(item); minX=Math.min(minX, b.left); minY=Math.min(minY, b.top); maxX=Math.max(maxX, b.right); maxY=Math.max(maxY, b.bottom); });
                            const pad = (80-71)/2; const extra = series === 'Esprit' ? 7.5 : 0;
                            return (
                                <g key={`f${i}`} pointerEvents="none">
                                    <rect x={minX-pad-extra} y={minY-pad-extra} width={maxX-minX+pad*2+extra*2} height={maxY-minY+pad*2+extra*2} fill="none" stroke={color} strokeWidth={series==='Esprit'?8:4} rx={series==='E3'?10:2} />
                                    <rect x={minX-pad+2} y={minY-pad+2} width={maxX-minX+pad*2-4} height={maxY-minY+pad*2-4} fill="none" stroke="#333" strokeWidth={1} rx={series==='E3'?8:1} />
                                </g>
                            );
                        })}
            ...
            <div className={`hidden p-8 bg-white w-full ${isPreviewMode ? 'block' : ''} print:block`}>
                ...
                {Object.keys(bom.byRoom).length === 0 ? <p className="text-gray-500 italic">Немає доданих елементів.</p> : 
                 Object.keys(bom.byRoom).map(roomName => (
                    <div key={roomName} className="mb-8 break-inside-avoid">
                        <h3 className="text-lg font-bold text-gray-800 mb-2 uppercase tracking-wide bg-gray-100 p-2 rounded">{roomName}</h3>
                        <table className="w-full text-sm border-collapse">
                            <thead><tr className="border-b border-gray-300"><th className="text-left py-2">Назва</th><th className="text-left py-2">Артикул</th><th className="text-right py-2">К-сть</th><th className="text-right py-2">Ціна</th><th className="text-right py-2">Сума</th></tr></thead>
                            <tbody>
                                {Object.values(bom.byRoom[roomName]).map((d, idx) => (
                                    <tr key={idx} className="border-b border-gray-100">
                                        <td className="py-2">{d.isAuto && <span className="text-blue-600 text-xs mr-1">[АВТО]</span>}{Object.keys(bom.byRoom[roomName]).find(key => bom.byRoom[roomName][key] === d)}</td>
                                        <td className="py-2 font-mono text-gray-500">{d.sku}</td>
                                        <td className="py-2 text-right font-bold">{d.count}</td>
                                        <td className="py-2 text-right text-gray-600">{d.price}</td>
                                        <td className="py-2 text-right">{d.count * d.price}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
